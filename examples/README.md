...
| [02-budget-ceiling](typescript/02-budget-ceiling/) | Cost ceilings — agent killed at $1.00 |
...
| [05-multi-agent](typescript/05-multi-agent/) | Multiple agents sharing a budget via `createRun()` |
...
| [02-budget-ceiling](python/02-budget-ceiling/) | Cost ceilings in Python |
...
- **`step`** — one tool call with timestamps, latency, cost, and args hash
- **`guard_event`** — a Fuze intervention (loop detected, budget exceeded, timeout)
- **`run_end`** — marks completion with final status and total cost
