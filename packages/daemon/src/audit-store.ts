...
function hashRecord(prevHash: string, fields: Record<string, unknown>): string {
...
export class AuditStore {
  ...
  private lastRunHash = 'genesis'
  private lastStepHash = 'genesis'
  private lastEventHash = 'genesis'
  private lastCompHash = 'genesis'
  ...
  async init(): Promise<void> {
    ...
    const lastRun = this.db.prepare('SELECT hash FROM runs ORDER BY rowid DESC LIMIT 1').get() as { hash: string } | undefined
    if (lastRun) this.lastRunHash = lastRun.hash

    const lastStep = this.db.prepare('SELECT hash FROM steps ORDER BY rowid DESC LIMIT 1').get() as { hash: string } | undefined
    if (lastStep) this.lastStepHash = lastStep.hash

    const lastEvent = this.db.prepare('SELECT hash FROM guard_events ORDER BY rowid DESC LIMIT 1').get() as { hash: string } | undefined
    if (lastEvent) this.lastEventHash = lastEvent.hash

    const lastComp = this.db.prepare('SELECT hash FROM compensation_records ORDER BY rowid DESC LIMIT 1').get() as { hash: string } | undefined
    if (lastComp) this.lastCompHash = lastComp.hash
  }
  ...
  async insertStep(step: Omit<DbStepRecord, 'prevHash' | 'hash'>): Promise<void> {
  ...
  async insertGuardEvent(event: Omit<DbGuardEventRecord, 'prevHash' | 'hash'>): Promise<void> {
    ...
    const hash = hashRecord(prevHash, colFields as Record<string, unknown>)
    this.db.prepare(
      `INSERT INTO guard_events (event_id, run_id, step_id, timestamp, event_type, severity, details_json, prev_hash, hash)
       VALUES (?,?,?,?,?,?,?,?,?)`
    ).run(
      event.eventId, event.runId, event.stepId ?? null, event.timestamp,
      event.eventType, event.severity, event.detailsJson, prevHash, hash,
    )
  ...
  async getRunSteps(runId: string): Promise<DbStepRecord[]> {
    const rows = this.db.prepare('SELECT * FROM steps WHERE run_id = ? ORDER BY step_number').all(runId) as Record<string, unknown>[]
    return rows.map(this._rowToStep)
  }

  async getRunGuardEvents(runId: string): Promise<DbGuardEventRecord[]> {
    const rows = this.db.prepare('SELECT * FROM guard_events WHERE run_id = ? ORDER BY rowid').all(runId) as Record<string, unknown>[]
    return rows.map(this._rowToEvent)
  }

  async listRuns(opts: ListRunsOpts = {}): Promise<RunRecord[]> {
    ...
    const rows = this.db.prepare(sql).all(...(params as SQLValue[])) as Record<string, unknown>[]
    return rows.map(this._rowToRun)
  }

  async countRuns(opts: Omit<ListRunsOpts, 'limit' | 'offset'> = {}): Promise<number> {
    ...
    const row = this.db.prepare(sql).get(...(params as SQLValue[])) as { count: number }
    return row.count
  }
  ...
  async verifyHashChain(): Promise<{ valid: boolean; brokenAt?: string }> {
    ...
    for (const row of runs) {
      const prev_hash = row['prev_hash'] as string
      const hash = row['hash'] as string
      if (prev_hash !== prev) return { valid: false, brokenAt: row['run_id'] as string }
      const immutable = {
      ...
      const expected = hashRecord(prev_hash, immutable as Record<string, unknown>)
      if (hash !== expected) return { valid: false, brokenAt: row['run_id'] as string }
      prev = hash
    }

    // Verify steps chain
    const steps = this.db.prepare('SELECT * FROM steps ORDER BY rowid').all() as Record<string, unknown>[]
    prev = 'genesis'
    for (const row of steps) {
      const { prev_hash, hash, ...fields } = row as { prev_hash: string; hash: string } & Record<string, unknown>
      if (prev_hash !== prev) return { valid: false, brokenAt: row['step_id'] as string }
      const expected = hashRecord(prev_hash, fields)
      if (hash !== expected) return { valid: false, brokenAt: row['step_id'] as string }
      prev = hash
    }

    // Verify guard_events chain
    const events = this.db.prepare('SELECT * FROM guard_events ORDER BY rowid').all() as Record<string, unknown>[]
    prev = 'genesis'
    for (const row of events) {
      const { prev_hash, hash, ...fields } = row as { prev_hash: string; hash: string } & Record<string, unknown>
      if (prev_hash !== prev) return { valid: false, brokenAt: row['event_id'] as string }
      const expected = hashRecord(prev_hash, fields)
      if (hash !== expected) return { valid: false, brokenAt: row['event_id'] as string }
      prev = hash
    }

    // Verify compensation_records chain
    const compRecords = this.db.prepare('SELECT * FROM compensation_records ORDER BY rowid').all() as Record<string, unknown>[]
    prev = 'genesis'
    for (const row of compRecords) {
      const { prev_hash, hash, ...fields } = row as { prev_hash: string; hash: string } & Record<string, unknown>
      if (prev_hash !== prev) return { valid: false, brokenAt: row['compensation_id'] as string }
      const expected = hashRecord(prev_hash, fields)
      if (hash !== expected) return { valid: false, brokenAt: row['compensation_id'] as string }
      prev = hash
    }
  ...
  async getRetentionStatus(): Promise<{ totalRuns: number; oldestRun: string; dbSizeBytes: number }> {
    const countRow = this.db.prepare('SELECT COUNT(*) as count FROM runs').get() as { count: number }
    const oldestRow = this.db.prepare('SELECT started_at FROM runs ORDER BY started_at LIMIT 1').get() as { started_at: string } | undefined
    return {
  ...
  async insertCompensationRecord(
    record: Omit<CompensationRecord, 'prevHash' | 'hash'>,
  ): Promise<void> {
    ...
    this.db.prepare(
      ...
      record.compensationId, record.runId, record.stepId, record.toolName,
      record.originalResultJson, record.compensationStatus,
      record.compensationStartedAt, record.compensationEndedAt,
      record.compensationError, record.escalated ? 1 : 0,
      prevHash, hash,
    )
    this.lastCompHash = hash
  }

  async getCompensationByRun(runId: string): Promise<CompensationRecord[]> {
    const rows = this.db
  ...
  async insertIdempotencyKey(record: IdempotencyRecord): Promise<void> {
    this.db.prepare(
      `INSERT OR IGNORE INTO idempotency_keys
       (key_hash, run_id, step_id, tool_name, args_hash, created_at, result_json)
       VALUES (?,?,?,?,?,?,?)`
    ).run(
      record.keyHash, record.runId, record.stepId, record.toolName,
      record.argsHash, record.createdAt, record.resultJson,
    )
  }

  async getIdempotencyKey(keyHash: string): Promise<IdempotencyRecord | null> {
    const row = this.db
  ...
  private _rowToRun(row: Record<string, unknown>): RunRecord {
    return {
  ...
  private _rowToStep(row: Record<string, unknown>): DbStepRecord {
    return {
  ...
  private _rowToEvent(row: Record<string, unknown>): DbGuardEventRecord {
    return {
  ...
  private _rowToCompensation(row: Record<string, unknown>): CompensationRecord {
    return {
      compensationId: row['compensation_id'] as string,
      runId: row['run_id'] as string,
      stepId: row['step_id'] as string,
      toolName: row['tool_name'] as string,
      originalResultJson: row['original_result_json'] as string | null,
      compensationStatus: row['compensation_status'] as CompensationRecord['compensationStatus'],
      compensationStartedAt: row['compensation_started_at'] as string | null,
      compensationEndedAt: row['compensation_ended_at'] as string | null,
      compensationError: row['compensation_error'] as string | null,
  ...
  private _rowToIdempotency(row: Record<string, unknown>): IdempotencyRecord {
    return {
