...
describe('ToolConfig', () => {
  it('returns built-in defaults when no config is provided', () => {
    ...
    expect(cfg.estimatedCost).toBe(0.01)
  ...
  it('returns tool-specific config when defined', () => {
    const tc = new ToolConfig({
      execute: { estimated_cost: 0.05, side_effect: true, max_calls_per_run: 10, timeout: 5000 },
    })
    const cfg = tc.getToolConfig('execute')
    expect(cfg.estimatedCost).toBe(0.05)
  ...
  it('falls back to [proxy.tools.default] for missing tool', () => {
    const tc = new ToolConfig({
      default: { estimated_cost: 0.02, side_effect: false, timeout: 15_000 },
    })
    const cfg = tc.getToolConfig('unknown_tool')
    expect(cfg.estimatedCost).toBe(0.02)
    expect(cfg.timeout).toBe(15_000)
  })

  it('tool-specific config overrides default', () => {
    const tc = new ToolConfig({
      default: { estimated_cost: 0.02, side_effect: false },
      query: { estimated_cost: 0.05 },
    })
  ...
  it('max_calls_per_run defaults to Infinity when not set', () => {
    const tc = new ToolConfig({ my_tool: { estimated_cost: 0.01 } })
